...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  //doan nay de lay tat ca thong tin cua board
  const board = useSelector(selectCurrentActiveBoard)

  //thanh vien trong card phai la tap con cua thanh vien trong board
  const FE_CardMembers = cardMemberIds.map(id => board.FE_allUsers.find(u => u._id === id))

  const handleUpdateCardMember = (user) => {
    // console.log(user)
    //tao 1 bien incomingMemberInfo de gui cho be, voi 2 thong tin chinh la userId va action la xoa khoi card hay them vao card
